...
// M23ObjectRegion2DDlg.cpp: 구현 파일
//

#include "pch.h"
#include "framework.h"
#include "M23ObjectRegion2D.h"
#include "M23ObjectRegion2DDlg.h"
...
// CM23ObjectRegion2DDlg 대화 상자



CM23ObjectRegion2DDlg::CM23ObjectRegion2DDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_M23OBJECTREGION2D_DIALOG, pParent),
...
void CM23ObjectRegion2DDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CM23ObjectRegion2DDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_LBUTTONDOWN()
END_MESSAGE_MAP()
...
// CM23ObjectRegion2DDlg 메시지 처리기

BOOL CM23ObjectRegion2DDlg::OnInitDialog()
{
...
void CM23ObjectRegion2DDlg::OnPaint()
{
	...
	else
	{
		...
		for (int y = 0; y < 3; y++) {
			for (int x = 0; x < 6; x++) {
				dc.Rectangle(x * 100, y * 100, (x + 1) * 100 + 1, (y + 1) * 100 + 1);
			}
		}
...
//  이 함수를 호출합니다.
HCURSOR CM23ObjectRegion2DDlg::OnQueryDragIcon()
{
...
void CM23ObjectRegion2DDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	// 정해진 6개 외의 영역 클릭 ==> 현재 상황에서 아무것도 바뀌지 않음 (선택한 항목 취소 아님)
	if (point.x < 600 && point.y < 300) {
